...
Native support for Parquet, JSON/NDJSON, and SQLite files across all modes (TUI, `-q` query, `-r`/`-c`/`-h` counts/headers, `-C` clipboard copy, `-S` split). Gzip-compressed CSV query support (`.csv.gz`, `.tsv.gz`) via DuckDB streaming decompression for `-q` mode — handles 100GB+ files without loading into memory; TUI open gives helpful error with suggested query command. DuckDB used as universal reader — no new crate dependencies. SQLite table selection via second positional arg (like XLSX sheet selection). Parallel row counting with rayon for large files. Clipboard paste (`-P`) auto-detects JSON vs CSV and converts accordingly, with graceful fallback for non-tabular JSON. File discovery extended to find all new format extensions. Extension stripping in SQL queries handles all new formats including `.csv.gz`. Piped-stdin detection improved to catch `cmd | lazycsv file`. Full error chain display (`{:#}`) for actionable error messages. Zero clippy warnings. 70 integration tests.
